...
func FindAssociatedTokenAddress(walletAddress, tokenMintAddress PublicKey, tokenProgramId string) (PublicKey, int, error) {
	tpId := TokenProgramID
	if !strings.EqualFold(tokenProgramId, "") {
		tpId = PublicKeyFromString(tokenProgramId)
	}

	seeds := [][]byte{}
	seeds = append(seeds, walletAddress.Bytes())
	seeds = append(seeds, tpId.Bytes())
