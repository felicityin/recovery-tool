...
func DerivePrivateKeyForPath(pk *ExtendedKey, path string) ([32]byte, []byte, error) {
	...
	for _, part := range parts {
		...
		_, extPk, err = DeriveChildKey(uint32(idx), harden, extPk, crypto.S256())
		if err != nil {
...
func DerivePublicKeyForPath(pk *ExtendedKey, path string) (*crypto.ECPoint, error) {
	...
	for _, part := range parts {
		...
		_, extPk, err = DeriveChildPubKey(uint32(idx), extPk, crypto.S256())
		if err != nil {
			return nil, fmt.Errorf("invalid BIP 32 path: %s", err)
		}
	}
	pubkeyPoint, err := crypto.NewECPoint(crypto.S256(), extPk.PublicKey.X, extPk.PublicKey.Y)
	if err != nil {
